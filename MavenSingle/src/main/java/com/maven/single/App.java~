...
public class App {
	...
	public static boolean fileExists() {
	        File f = new File("/usr/local/apache-maven-3.3.9/MavenSingle/target/Output.txt");
	...
	public static void readFile() throws IOException {
		BufferedReader br = new BufferedReader(new FileReader("/usr/local/apache-maven-3.3.9/MavenSingle/target/Output.txt"));
		int rowCount = 1;
		int maxColCount = 1;
		int currentRow = 0;
	   ...
	   if(!fileExists()) {
		System.out.println("No file exists");
	   	BufferedWriter output = null;
			try {
        		    File file = new File("/usr/local/apache-maven-3.3.9/MavenSingle/target/Output.txt");
        		} catch ( Exception e ) {
            			e.printStackTrace();
        		} finally {
